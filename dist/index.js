...
function secureStore() {
  try {
    const required = require("expo-secure-store");
    const module2 = required.default ?? required;
    if (typeof module2.getItemAsync === "function" && typeof module2.setItemAsync === "function") {
      return module2;
    }
  } catch {
  }
...
function mobilePlatform() {
  return import_react_native.Platform.OS === "android" ? "android" : "ios";
}
...
function collectDeviceContext() {
  const context = {};
  try {
    const required = require("expo-device");
  ...
  const runtime = import_react_native.Platform;
...
async function sendRegistration(token) {
  ...
  try {
    ...
    const bundleId = import_react_native.Platform.OS === "ios" ? expoConfig?.ios?.bundleIdentifier : expoConfig?.android?.package;
    if (bundleId) {
